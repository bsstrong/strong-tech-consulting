...
- assignment management is available
...
- handoff from Broker creation/assignment to PR 1's Assigned Clients control
...
1. add Broker
2. select that person and manage Assigned Clients through PR 1

A newly invited Broker with zero assignments sees the assigned-specific empty
state. Optionally open the Assigned Clients dialog immediately after creation;
do not duplicate assignment storage in this PR.
...
- warn that changing primary role replaces the current primary permission
  template
- use PR 1's Assigned Clients management without a second implementation
...
- Broker handoff opens or clearly points to Assigned Clients
...
4. `feat(web): expose Broker primary role workflows`
5. `test(auth): prove Broker least privilege`
6. `docs(api): refresh Broker assignment contracts` if needed
...
7. Assign two exact Clients through People & Access.
...
11. Remove one assignment and confirm it disappears on the next request.
12. Change Broker primary role to Carrier Member and confirm the pointer clears
    intentionally and CarrierMember capabilities become effective.
13. Recheck an existing pre-migration Carrier Member and Carrier Admin; their
...
- screenshots of matrix, Add Team Member, People & Access, and Broker directory
